---
title: "Kingmaker"
description: "The land rush is on! Sent south by Brevoy, the heroes have the unenviable task of venturing into the infamous Stolen Lands and annexing the territory, facing down monsters, bandits, and worse. It's hard enough to conquer territory—but does a ragtag band of adventurers have what it takes to found and defend a burgeoning kingdom from the terrors of the wild? "
campaignType: "adventure-path"
levelRange:
  min: 1
  max: 20
parts: 6
tags: ["Kingdom Building", "Epic", "Sandbox", "Political", "Stolen Lands", "Classic"]
publisher: "Paizo"
heroImage: "../../../src/assets/kingmaker.jpg"
status: "available"
sessionDuration: "3-4 hours"
playersMin: 4
playersMax: 6
publishDate: 2010-03-31
trailer: 'ClbWYVIBK9A'
logoUrl: 'https://pathfinderwiki.com/w/images/thumb/b/ba/Kingmaker_logo.webp/700px-Kingmaker_logo.webp'
wikiEntry: 'https://pathfinderwiki.com/wiki/Kingmaker_(adventure_path)'
---

## Overview

The land rush is on! Sent south by Brevoy,
the heroes have the unenviable task of venturing into the infamous Stolen Lands and annexing the territory,
facing down monsters, bandits, and worse.
It's hard enough to conquer territory—but does a ragtag band of adventurers have what it takes to found and defend a burgeoning kingdom from the terrors of the wild?

The Kingmaker Adventure Path takes the heroes from encounters with mysterious bandit lords and barbaric raiders through
the trials and tribulations of developing and defending their new settlement in the notoriously lawless River Kingdoms.
Yet when war comes to the Stolen Lands in earnest, it's up the heroes to take up a mystical blade and stand tall against the horrors of man,
beast, and strange creatures more dangerous than either...

Note that if you've played the video game "Kingmaker" you'll unfortunately know most of the story, even if the adventure path
slightly deviates from the video game.

## What to Expect

- **Kingdom Building**: Build and manage your own nation
- **Full 1-20 Campaign**: Complete progression from novice to legend
- **Sandbox Exploration**: Explore and chart the Stolen Lands
- **Political Intrigue**: Navigate rulership and diplomacy
- **Classic Adventure**: Updated version of beloved campaign
- **Management Systems**: Balance adventuring with governance
- **Legacy Building**: Create a lasting realm

## Perfect For

This campaign is ideal for players who enjoy:
- Kingdom building and management gameplay
- Sandbox exploration and player-driven stories
- Full 1-20 epic campaigns
- Political intrigue and leadership roleplay
- Balancing adventure with realm management
- Classic Pathfinder content updated for 2E
- Long-term campaigns with lasting impact

## Campaign Structure

The adventure unfolds across six comprehensive books:

1. **Part 1** (Levels 1-3) - Exploring and claiming the Stolen Lands
2. **Part 2** (Levels 4-6) - Kingdom founding and early threats
3. **Part 3** (Levels 7-10) - Kingdom growth and fey complications
4. **Part 4** (Levels 11-14) - Political intrigue and external threats
5. **Part 5** (Levels 15-17) - Epic challenges to your realm
6. **Part 6** (Levels 18-20) - Final threats and legendary rulers

## What Makes This Campaign Special

- Iconic kingdom-building gameplay
- Complete 1-20 adventure path
- Sandbox exploration and player agency
- Balance of adventure and realm management
- Fey politics and wilderness exploration
- Updated and improved for Pathfinder 2E
- Create a lasting legacy as nation founders
- One of the most beloved Pathfinder campaigns
