---
title: "Seven Dooms for Sandpoint"
description: "The town of Sandpoint has seen more than its fair share of danger and trouble over the years, including harrowing fires, prolific serial killers, goblin raids, and attacks by giants and dragons, but what faces the so-called Light of the Lost Coast now is its greatest threat yet! Something sinister has been manipulating events all along, and now a new band of heroes must step in to save this legendary small town from seven deadly dooms! "
campaignType: "adventure-path"
levelRange:
  min: 4
  max: 11
parts: 3
tags: ["Sandpoint", "Classic Location", "Mystery", "Community Defense", "Nostalgia"]
publisher: "Paizo"
heroImage: "../../../src/assets/seven-dooms-of-sandpoint.jpg"
status: "available"
sessionDuration: "3-4 hours"
playersMin: 4
playersMax: 6
publishDate: 2024-03-08
trailer: 'aazeY5gHZ74'
logoUrl: 'https://pathfinderwiki.com/w/images/thumb/f/f2/Seven_Dooms_for_Sandpoint_softcover.jpg/500px-Seven_Dooms_for_Sandpoint_softcover.jpg.webp'
wikiEntry: 'https://pathfinderwiki.com/wiki/Seven_Dooms_for_Sandpoint'
---

## Overview

Back where it all began!

This special, double sized volume presents a full Adventure Path in a single massive 200 page book to celebrate the 200th
volume of the Pathfinder Adventure Path!

Seven Dooms for Sandpoint brings players back to one of Pathfinder's most iconic locations.
The peaceful coastal town of Sandpoint, familiar to many from Rise of the Runelords, now faces its greatest threat:
seven interconnected dooms orchestrated by sinister forces. As mid-level heroes, you'll investigate these mounting catastrophes,
uncover the conspiracy behind them, and fight to save the town and its beloved residents from total destruction.

## What to Expect

- **Return to Sandpoint**: Revisit this iconic Pathfinder location
- **Seven Interconnected Threats**: Multiple doom scenarios to investigate
- **Community Stakes**: Fight to save townspeople you care about
- **Mystery & Investigation**: Uncover the conspiracy behind the dooms
- **Escalating Danger**: Each doom builds toward catastrophe
- **Memorable NPCs**: Interact with classic Sandpoint residents

## Perfect For

This campaign is ideal for players who enjoy:
- Returning to classic Pathfinder locations
- Community-focused storytelling
- Mystery and investigation alongside combat
- Mid-level adventures with meaningful stakes
- Protecting people and places that matter
- Nostalgia for Rise of the Runelords

## Campaign Structure

The adventure unfolds across three interconnected books:

1. **Part 1** (Levels 4-6) - First dooms emerge in Sandpoint
2. **Part 2** (Levels 7-9) - Conspiracy deepens, stakes escalate
3. **Part 3** (Levels 10-11) - Final dooms and climactic resolution

## What Makes This Campaign Special

- Return to beloved Sandpoint with fresh threats
- Seven unique doom scenarios create varied challenges
- Strong emotional investment in community survival
- Rich callback to Pathfinder history
- Balance of investigation, social encounters, and combat
- Meaningful connections with NPCs and location
- Perfect for fans of Rise of the Runelords
